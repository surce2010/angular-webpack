...
module.exports = function(ngModule) {
    ngModule.factory('httpMethod', ['$http', '$q', function($http, $q) {
        ...
        httpMethod.querySupplierByCond = function(param) {
            let defer = $q.defer();
            $http({
                url: CONFIG.API.SERVER_ADDRESS + '/privilege/profile/queryOperationSpecType.action',
                method: 'POST',
        ...
        if (CONFIG.API.USEMOCK) {
            Mock.mock(CONFIG.API.SERVER_ADDRESS + '/privilege/profile/queryOperationSpecType.action', {
                'rsphead': 's',
                'success': true, //是否成功
                'code': null,
                'msg': null, //失败信息
